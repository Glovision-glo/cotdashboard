...
// loadMap(slat,slng);
 ...
 function loadMap(lat,lng){
     ...
     // attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>'}).addTo(map);
       map = new MapmyIndia.Map('mapcontainer', {center: [lat, lng], zoomControl: true, hybrid: true});
     //map.invalidateSize();
  }else{
...
var intervelID;
 function replay1(){
            ...
            map.off();
            map.remove();
               map = new MapmyIndia.Map('mapcontainer', {center: [center[2],center[3]], zoomControl: true, hybrid: true});
           map.invalidateSize();
             L.control.zoom({
              position:'topright'
          }).addTo(map);
