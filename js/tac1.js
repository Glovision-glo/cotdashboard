...
var overspeed=0;
 ...
 //map boundaries
if(maploadcount==0 && (window.localStorage.getItem('accountID')=="gvk-up-108" || window.localStorage.getItem('accountID')=="gvk-up-102" || window.localStorage.getItem('accountID')=="als")){
...
/*
	...
	function startAndStopOfVehiclePosition(vehicleNo,map,infostatus){
	  ...
	  if(UrlExit(image)){
	  	  ...
	  	  var discription='';
	  	var Account=window.localStorage.getItem('accountID');
	  	  if(Account.indexOf("gvk")>-1 || Account.indexOf("gog")>-1 || Account.indexOf("khil")>-1  || Account.indexOf("als")>-1){
...
//populate baselocations list
		...
		//	alert(vehicleAvailable[vehicleIndx][11]+" 12:"+vehicleAvailable[vehicleIndx][12]+" 13:"+vehicleAvailable[vehicleIndx][13]+" 14:"+vehicleAvailable[vehicleIndx][14]+" 4:"+vehicleAvailable[vehicleIndx][4]+" 5:"+vehicleAvailable[vehicleIndx][5]+" 6:"+vehicleAvailable[vehicleIndx][6]+" 7:"+vehicleAvailable[vehicleIndx][7]+" 8:"+vehicleAvailable[vehicleIndx][8]+" 9:"+vehicleAvailable[vehicleIndx][9]+" 10:  "+vehicleAvailable[vehicleIndx][10]);
			...
			// vehiclesType="ambulance";
	         ...
	         if(Account.indexOf("gvk")>-1 || Account.indexOf("gog")>-1 || Account.indexOf("khil")>-1  || Account.indexOf("als")>-1){//Account=="gvkrajasthan"
	   ...
	   if((typeof(vehicleAvailable[vehicleIndx][6]) !== 'undefined') && (vehicleAvailable[vehicleIndx][6] !== null)) {
			          ...
			          discription="<td>"+(typeof(result[0])== 'undefined'?'':result[0]).replace(']','')+"</td><td style='width:15'>"+(typeof(result[1])== 'undefined'?' ':result[1]).replace(']','')+"</td>";
                                  discription2="<td>"+(userID=="monitoring"|| userID=="tracking"?'':vehicleAvailable[vehicleIndx][9])+"</td><td>"+vehicleAvailable[vehicleIndx][3]+"</td><td>"+vehicleAvailable[vehicleIndx][12]+"</td><td>"+vehicleAvailable[vehicleIndx][5]+"</td>";
			 ...
			 if(vehiclesType==''){vehiclesType='ambulance'; }     
			if(vehicleAvailable[vehicleIndx][1]=="61714" && (selectedGroup.toUpperCase()==vehiclegroup.toUpperCase() || selectedGroup=="selectall" || vehicleCaseStatus==showmap)){
				Status="Iggniton On";
				Symbol='RunningE';
				countInMotion++;
				...
				//	running=running+"<tr><td><u><a  onClick='vehicleRoute("+vehicleIndx+",1)'>"+vehicleAvailable[vehicleIndx][0].toUpperCase()+"</a></u></td>"+discription+"<td><a  onClick='vehicleRoute("+vehicleIndx+",1)'><img src='"+image+"' align='center' height='20'></a></td></tr>";
                           ...
                           if(vehicleAvailable[vehicleIndx][5]>=overspeed){
                              ...
                              if((showmap!='all' && showmap!="online" && showmap!="offline" && showmap!='idle') && showmap==caselist){
                              ...
                              }else if(showmap=='all' || showmap=="online" || showmap=="offline" || showmap=='idle'){ 
                                running=running+"<tr><td><a  onClick='vehicleRoute("+vehicleIndx+",1)'><img src='"+image+"' align='center' height='20'></a></td>"+discription+"<td style='background-color:"+color+";color:white'><p onClick='vehicleRoute("+vehicleIndx+",1)'  >"+vehicleAvailable[vehicleIndx][0].toUpperCase()+"</p> </td>"+discription2+$routeUrl+"</tr>";
			...
			}else if((vehicleAvailable[vehicleIndx][1]=="61718" || vehicleAvailable[vehicleIndx][1]=="61720" || vehicleAvailable[vehicleIndx][1]=="61717") && (selectedGroup.toUpperCase()==vehiclegroup.toUpperCase() || selectedGroup=="selectall" || vehicleCaseStatus==showmap)){
				Status="Idle";
				Symbol='IdleE';
				countIdle++; 
	     	   	var image="images/"+vehiclesType+"/"+Symbol+".png";
		...
		//		idle=idle+"<tr><td><u><a  onClick='vehicleRoute("+vehicleIndx+",1)'>"+vehicleAvailable[vehicleIndx][0].toUpperCase()+"</a></u></td>"+discription+"<td><a  onClick='vehicleRoute("+vehicleIndx+",1)'><img src='"+image+"' align='center' height='20'></a></td></tr>";
                ...
                //     if(selectedGroup==vehiclegroup || selectedGroup=="selectall")
                    image="images/off.gif";
                          var color="#D7DF01";
                            var caselist='';
                                   if(vehicleCaseStatus=='assigned'){caselist='towardsscene';color='#8e44ad';casemanagerconunt[1][0]=casemanagerconunt[1][0]+1;}
                                  else  if(vehicleCaseStatus=='inzone'){caselist='atscene';color='#3498db';casemanagerconunt[1][1]=casemanagerconunt[1][1]+1;}
                                   else if(vehicleCaseStatus=='tohospital'){caselist='towardshospital';color='#dc7633';casemanagerconunt[1][3]=casemanagerconunt[1][3]+1;}
                                  else{casemanagerconunt[1][2]=casemanagerconunt[1][2]+1;caselist='readytotakecase-idle';}
			...
			}*/else if(vehicleAvailable[vehicleIndx][1]=="0000" && (selectedGroup.toUpperCase()==vehiclegroup.toUpperCase() || selectedGroup=="selectall" || vehicleCaseStatus==showmap)) {
          ...
          $routeUrl="<td>"+vehicleAvailable[vehicleIndx][14]+"</td><td><input type='button' onClick=routeTracking('"+vehicleAvailable[vehicleIndx][0]+"') value='History' style='width:70px' class='button-clear'></td>";
                  ...
                  //if(selectedGroup==vehiclegroup || selectedGroup=="selectall")
                      var color="#F781D8";
                      var caselist='';
                                   if(vehicleCaseStatus=='assigned'){caselist='offline-caseassigned';color='#DF0101';casemanagerconunt[2][0]=casemanagerconunt[2][0]+1;}
                                  else if(vehicleCaseStatus=='inzone'){caselist='offline-caseassinged';color='#DF0101';casemanagerconunt[2][1]=casemanagerconunt[2][1]+1;}
                                    else if(vehicleCaseStatus=='tohospital'){caselist='offline-caseassigned';color='#dc7633';casemanagerconunt[2][3]=casemanagerconunt[2][3]+1;}
                                   else{casemanagerconunt[2][2]=casemanagerconunt[2][2]+1;caselist='offline-casenotassigned';}
			...
			}else if(selectedGroup.toUpperCase()==vehiclegroup.toUpperCase() || selectedGroup=="selectall" || vehicleCaseStatus==showmap){
				Status="Idle";
				Symbol='stoppedE';
