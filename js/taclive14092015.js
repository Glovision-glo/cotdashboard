...
/////////////////vehicle Status.js///////////////
	...
	function allVehicleStatus() {
	    ...
	    for(var i=0;i<vehicleWithEventData.length;i++){
	  	...
	  	//	alert(image+" 1");
	  	   ...
	  	   //alert(image);
	  	if(showvehicles==showstatus || showvehicles=='all'){ 
	  	     ...
	  	     vehicleAvailable[vehicleAvailableIndex][9]=vehicleWithEventData[i][lastEventOfVehicle][14];
	            ...
	            //allVehicleRoute(map); //too late load all vehicles
	       	    ...
	       	    // Add the circle for this city to the map.
	       	     ...
	       	     var res=globalVehicles1[i].split("][");
		...
		if(vehicleWithEventData[vehicleNo].length>0){
	        ...
	        if(typeview==3){
	        	...
	        	for(var index=0;index<VehiclesArrayOrVehicle.length;index++){
	        		startAndStopOfVehiclePosition(VehiclesArrayOrVehicle[index],map);
	        	}
	        
	        
	        }else{
	        	startAndStopOfVehiclePosition(vehicleNo,map);
	        }
	...
	function startAndStopOfVehiclePosition(vehicleNo,map){
	  ...
	  if(UrlExit(image)){
	     ...
	     var vehicleTitle="<h4> vehicle Id:  "+(vehicleWithEventData[vehicleNo][lastEventOfVehicle][9].toUpperCase())+"<br>DateTime :<b>"+vehicleWithEventData[vehicleNo][lastEventOfVehicle][4]+"</b><br> Lat/Long :<b>"+vehicleWithEventData[vehicleNo][lastEventOfVehicle][6]+"/"+vehicleWithEventData[vehicleNo][lastEventOfVehicle][7]+"</b><br>Speed :<b>"+vehicleWithEventData[vehicleNo][lastEventOfVehicle][3]+" Kmph</b><br>  Adress : <b>"+vehicleWithEventData[vehicleNo][lastEventOfVehicle][8]+"</b><br>  Odometer Reading: <b>"+vehicleWithEventData[vehicleNo][lastEventOfVehicle][5]+" Km</b><br>"+discription+"</h4>";
	     putPushpinVehicles(parseFloat(vehicleWithEventData[vehicleNo][lastEventOfVehicle][6]),parseFloat(vehicleWithEventData[vehicleNo][lastEventOfVehicle][7]),map,image,vehicleTitle,vehicleWithEventData[vehicleNo][lastEventOfVehicle][9],'yes');
	...
	function availabilityVehicles(){
		...
		var h1=(screen.height/2)+(screen.height/20);
			var table="<div class='hello' style='height:"+h1+";width:100%;border:solid 2px orange;background: white;overflow: scroll;overflow-y:scroll;'><table border='1' align='center' id='search'><tbody>";
			var table1="<div class='hello' style='height:"+h1+";width:100%;border:solid 2px orange;background: white;overflow: scroll;overflow-y:scroll;'><table border='1' align='center'><tbody>";
	   ...
	   var uri = window.location.toString();
		...
		for(var vehicleIndx=0;vehicleIndx<vehicleAvailable.length;vehicleIndx++){
			...
			// vehiclesType="ambulance";
	         ...
	         if(Account.indexOf("gvk")>-1){//Account=="gvkrajasthan"
	        	  // alert(vehicleAvailable[vehicleIndx][6]);
	        	
	        	  var head="<table border='1' align='center' style='width:100%'><thead><tr><th>Vehicle Id</th><th>Phone </th><th>Base Location</th><th>District </th><th>Status</th></tr></thead></table><input id='ss' name='filter' onkeyup='filter(this,0)' size='15'  type='text' placeholder='Vehilce Search'><input id='ad' name='filter' onkeyup='filter(this,2)' size='15' type='text' placeholder='Base Locatoin Search'><input id='ad2' name='filter' onkeyup='filter(this,3)' size='15' type='text' placeholder='District Search'>";
	        	  //<th>Address </th>
	        	  ...
	        	  if((typeof(vehicleAvailable[vehicleIndx][6]) !== 'undefined') && (vehicleAvailable[vehicleIndx][6] !== null)) {
			          ...
			          discription="<td>"+vehicleAvailable[vehicleIndx][9]+"</td><td>"+(typeof(result[1])== 'undefined'?' ':result[1]).replace(']','')+"</td><td>"+(typeof(result[0])== 'undefined'?'':result[0]).replace(']','')+"</td>";
		...
		if(showmap=='all' ||showmap=='clear' ){table=table+running+idle+stopped+offline+"</tbody></table><div>";}
		...
		//table=table+"</tbody></table><div>";
	     ...
	     /*var flagTable = "<table>";
	    	 ...
	    	 */
	     var flagTable = "<table>";
         flagTable +=  "<tr><td>&nbsp&nbspTotal Vehicle &nbsp&nbsp:</td><td> "+totalvehicle+"</td></tr>";
         flagTable +=  "<tr><td>&nbsp&nbspOnline Vehicle :</td><td> "+onlineVehilce+"</td></tr>";
         flagTable +=  "<tr><td></td><td>Running :<font color='green'> "+countInMotion+"</font></td></tr>";
         
         document.getElementById("online").value='Running('+countInMotion+')';
         document.getElementById("idle").value='Idle('+(countIdle+countStopped)+')';
         document.getElementById("offline").value='Offline('+countRepair+')';
         
         document.getElementById("reset").value='All('+totalvehicle+')';
         var Account=window.localStorage.getItem('accountID');
         if(Account.indexOf("gvk")>-1){
        	 flagTable += "<tr><td></td><td>Idle: <font color='#FF9933'>"+(countIdle+countStopped)+"</font></td></tr>";
    	 ...
    	 flagTable += "<tr><td>&nbsp&nbspOffline &nbsp&nbsp&nbsp&nbsp&nbsp : </td><td>"+countRepair+"</td></tr>";
