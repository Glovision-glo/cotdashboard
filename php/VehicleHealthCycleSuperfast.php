...
$healthDays=intval(abs($toDate-$fromDate)/86400)+1;
    ...
    if($crone=="yes"){  //if request from crone
       ...
       $gtsconnect = mysqli_connect('localhost', 'root','gl0v1s10n' ) or die ("Unable to connect to the database: " . mysqli_error($gtsconnect));
       ...
       if($crone=="yes"){
          ...
          for($deviceIndx=0;$deviceIndx<count($totalDevices);$deviceIndx++){
              ...
              for($dateIndx=0;$dateIndx<intval($healthDays);$dateIndx++){
               ...
               mysqli_query($tacconnect,$query) or die(mysqli_error($tacconnect)); 
                $fromDump=$fromDump+86400;
              $toDump=$toDump-86400;
              }
          }     
...
$(document).ready(function () {
    $('#sort').tablesorter();
        ...
        if($type=="invoice"){
             ...
             $ttable=$ttable.'<th>Active Days</th></thead><tbody>';
        }
       ...
       for($deviceIndx=0;$deviceIndx<count($totalDevices1);$deviceIndx++){
          ...
          $imei='';
           ...
           if (strpos($model,"TFT") !== false || strpos($model,"tft") !== false ) {
                ...
                $tfttrackers=$tfttrackers.'<tr></td><td>'.$accountID.'</td><td>'.$group.'</td><td>'.strtoupper($totalDevices1[$deviceIndx]).'</td><td>'.$model.'</td><td>'.$imei.'</td><td>'.$NoOfDaysActive.'</td><td>'.$rent.'</td></tr>';
                if($NoOfDaysActive>0){
                     $TFTVehicleCount++;
                }
                ...
                $wtdtrackers=$wtdtrackers.'<tr></td><td>'.$accountID.'</td><td>'.$group.'</td><td>'.strtoupper($totalDevices1[$deviceIndx]).'</td><td>'.$model.'</td><td>'.$imei.'</td><td>'.$NoOfDaysActive.'</td><td>'.$rent.'</td></tr>';
                 if($NoOfDaysActive>0){
                     $WTDVehicleCount++;
                }
           }
          ...
          if($type !="invoice"){
              ...
              if($streanth==intval($healthDays)){
                  $working=$working.'<tr><th>'.strtoupper($totalDevices1[$deviceIndx]).'</th><th style="color:green">Working</th>'.$row.'<td>'.$NoOfDaysActive.'</td></tr>';
              
                $workingpercent=$workingpercent.'<tr><th>'.strtoupper($totalDevices1[$deviceIndx]).'</th><th style="color:green">Working</th>'.$percentrow.'<td>'.$NoOfDaysActive.'</td></tr>';

 
             }else if($toweak==intval($healthDays)){
                  $notWorking=$notWorking.'<tr><th>'.strtoupper($totalDevices1[$deviceIndx]).'</th><th style="color:red">Not Working</th>'.$row.'<td>'.$NoOfDaysActive.'</td></tr>';
                 $notWorkingpercent=$notWorkingpercent.'<tr><th>'.strtoupper($totalDevices1[$deviceIndx]).'</th><th style="color:red">Not Working</th>'.$percentrow.'<td>'.$NoOfDaysActive.'</td></tr>';
                   ...
                   $gaps=$gaps.'<tr><th>'.strtoupper($totalDevices1[$deviceIndx]).'</th><th style="color:#04B404">Gaps</th>'.$row.'<td>'.$NoOfDaysActive.'</td></tr>';
...
function getModelImei($vehicleID,$accountID){
          ...
          $connect = mysqli_connect("track1.glovision.co","gts", "opengts") or die ("Unable to connect to the database: " . mysqli_error($connect));
   mysqli_select_db($connect,"gts");
        
       $devices = mysqli_query($connect,$deviceQuery) or die(mysqli_error($connect)); 
       $result='';
       while($row = mysqli_fetch_assoc($devices))
       {
