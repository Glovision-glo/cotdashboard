   ...
   // $summaryIndx=0;
    ...
    for($devindx=0;$devindx<count($deviceArrya);$devindx++) {
     ...
     //  $sql = "Select accountID,deviceID,address,from_unixtime( timestamp, '%d-%m-%Y %H:%i:%s' ) as time1,statusCode,latitude,longitude,geozoneID,speedKPH from EventData where accountID='$accountID' and deviceID='".$deviceArrya[$devindx][4]."' and timestamp BETWEEN $m AND $m1";
        $s='';
        $ssdiv=$deviceArrya[$devindx][4];
        $eventIncreament='yes';  
      if (mysqli_multi_query($gtsconnect, "call eventdata($fromDate,$toDate,'$accountID','$ssdiv',1800)"))
     {
      do
        {
...
if($indx>=2){ //delete drift event compare 1 to second and first to third.
                 ...
                 $dis1topresent=distance1($row[5],$row[6], $data[$indx-2][5], $data[$indx-2][6]);
           ...
           if($indx>0 && $row[5]!= 0 && $row[6]!=0 && $data[$indx-1][5]!=0 && $data[$indx-1][6]!=0) {
                      $diffdis=distance1($row[5],$row[6], $data[$indx-1][5], $data[$indx-1][6]);
                ...
                $data[$indx][7]=distance1($row[5],$row[6], $data[$indx-1][5], $data[$indx-1][6]);
