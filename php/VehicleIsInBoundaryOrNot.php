...
$table="<table style='border: 1px solid black;'><tr><th>Device ID</th> <th>Base Location </th><th>Driver Contact</th> <th>Assigned District </th><th>Current District </th><th>Time</th></tr>";
getLastLocation();
...
if($lastLocaton[$x][10]!="" && $lastLocaton[$x][11]!=""){
   ...
   if($cd!=""){
      ...
      $lastLocaton[$x][6]=strtolower(trim(str_replace("br","",preg_replace("/[^a-zA-Z]/", "",  $lastLocaton[$x][6]))));
      $percent=wordMatchingPercent($cd,$lastLocaton[$x][6]);
     // if(strtolower(trim($cd))!=strtolower(trim($lastLocaton[$x][6]))){
      if($percent<80){//vehicle boundary check based on percent
...
$mailperAccount[0]="shivaprasad@glovision.co";
#$mailperAccount[1]="chethanr@glovision.co";
#$mailperAccount[2]="tushar_shroff@emri.in";

//mailSendToAccount($mailperAccount,$table,$accountID,"Vehicles Crossing Assigned District Report");
