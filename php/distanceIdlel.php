   ...
   // $summaryIndx=0;
    ...
    for($devindx=0;$devindx<count($deviceArrya);$devindx++) {
        ...
        $gtsconnect = mysqli_connect($gtsserver, $gtsusername, $gtspassword) or die ("Unable to connect to the database1 : " . mysqli_error());
         $selecteddbdate=date('m-Y',$fromDate);
...
error_log($sql);
         error_log( "------------------------------------------------------------------");
        $qry_result = mysqli_query($gtsconnect, $sql) or die(mysqli_error($gtsconnect));
        $data=array(array());
...
if($indx>=2){ //delete drift event compare 1 to second and first to third.
        ...
        //              echo (abs(strtotime($data[$indx][3])-strtotime($data[$indx-1][3]))/60).'  dis:  '.$diffdis.'  '.$row['latitude'].'  '.$row['longitude'].'  '.$data[$indx][3].'<br>';
                     //   if(abs(strtotime($data[$indx][3])-strtotime($data[$indx-1][3]))/60>4 && $diffdis>5 && ($accountID=="gvkgjtest" || $accountID=="goghealth")){
      ...
      //     echo $data[$indx][1].'   '.(abs(strtotime($data[$indx][3])-strtotime($data[$indx-1][3]))/60).'  '.$diffdis.'  '.$row['latitude'].'  '.$row['longitude'].'  '.$data[$indx-1][5].' '.$data[$indx-1][6].' callllllllllllllllllll dis<br>';
         ...
         // echo $data[$indx][7];              
            if($accountID != "gvkrajasthan"){
                    $difftimes=abs(strtotime($data[$indx][3])-strtotime($data[$indx-1][3]))/60;
                    if(is_nan($data[$indx][7]) || (intval($data[$indx][7])>5 && $difftimes<5)){//if distnace greater than 2 km with 5 minits then distnace=0 bzc drift point  
                    $eventIncreament="no"; 
                    //delete duplicate events
                   //           echo $data[$indx][7].'  '.$difftimes.'-------------------------<br>';    
                    $data[$indx][7]=0;
                    }
               }
             }
               /* if(is_nan($data[$indx][7]) || (intval($data[$indx][7])>10 && $difftimes<5)){//if distnace greater than 2 km with 5 minits then distnace=0 bzc diff point  
                      $data[$indx][7]=0;
                }*/
               // if ($data[$indx][7] > 0) 
