<?php
    ...
    $ambulanceID=$_GET['ambulanceID'];
    $travelDistance='';//$_GET['travelDistance'].' Kms';
    $result= gethighwaydata($ambulanceID,$accountID);
    $fromDate=$result[0];
    $toDate=$result[6];
    $vehicleID=$result[8];
    $caseInfo=$result[0].'^'.$result[1].'^'.$result[2].'^'.$result[3].'^'.$result[4].'^'.$result[5].'^'.$result[6].'^'.$result[7].'^'.$result[8].'^'.$result[9].'^'.$result[10].'^'.$result[11].'^';
    
    $data=$result[0]."^".$result[2]."^".$result[3]."^".$result[4]."^*".$result[0]."^".$result[5]."^".$result[6]."^".$result[7]."^*".$result[0]."^".$result[8]."^".$result[9]."^".$result[10]."*^";
...
function gethighwaydata($ambuID,$accountID){
...
// Full texts 	accountID 	ambulanceID 	ambulanceContact 	district 	routeName 	actualPositionLongitude 	actualPositionLatitude 	highwayLandmark 	nearestHealthCentre 	nearestHCLatitude 	nearestHCLongitude 	shortestRoutetoHC 	shortRouteHCLatitude 	shortRouteHCLongitude 	actualPositionKMS 	nearestHCDistance 	nearestHCTime



      $Query="select * From lucknowExpressway where ambulanceID='$ambuID' and accountID='$accountID'";
      $rs =mysqli_query($tacconnect,$Query) or die ("Query error1: " . mysqli_error($tacconnect));
      while($row = mysqli_fetch_assoc($rs)) {
          $result[0]=$row['ambulanceID'];
          $result[1]=$row['ambulanceContact'];
          $result[2]=$row['routeName'];
          $result[3]=$row['actualPositionLatitude'];
          $result[4]=$row['actualPositionLongitude']; 
              
          $result[5]=$row['highwayLandmark'];
          $result[6]=$row['nearestHCLatitude'];
          $result[7]=$row['nearestHCLongitude'];
          $result[8]=$row['shortestRoutetoHC'];
          $result[9]=$row['shortRouteHCLatitude'];
          $result[10]=$row['shortRouteHCLongitude'];
          $result[11]=$row['actualPositionKMS'];
       ...
       while($row = mysqli_fetch_assoc($rs)) {
          ...
          if($diffassignarrivel<200 && $basestart=="no"){
           ...
           if($statusinfo=="Seen Arrival"){ 
               ...
               $eventData[$i][14]='Time:'.date('d-m-Y H:i:s',$result[2])."<br>Seen Arrival";        
           }//main power off/on
           if($statusinfo=="Seen Depart"){
               $eventData[$i][13]=$statusinfo;
               $eventData[$i][14]='Time:'.date('d-m-Y H:i:s',$result[3])."<br>Seen Depart";
           }//overspeed event
       ...
       <script>
           ...
           function initialise4() {
             ...
             if(evntdata[2]!=0){
              ...
              if(evntdata2[2]!=0){
                      vehicleTrack(evntdata2[2],evntdata2[3],lineSymbol,map,0,"yes","h0",evntdata2[1]); 
                      if(evntdata3[2]!=0){
              ...
              if(evntdata3[2]!=0){
                      vehicleTrack(evntdata3[2],evntdata3[3],lineSymbol,map,0,"yes","h0",evntdata3[1]); 
               } 
