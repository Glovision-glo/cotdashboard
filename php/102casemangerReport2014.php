...
.tablecss {
     ...
     $tacconnect = mysqli_connect($tacserver, $tacusername, $tacpassword) or die ("Unable to connect to the database: " . mysqli_error($tacconnect));
      ...
      $Query="select * From caseData where assignedTime between '$fromDate' and '$toDate'";
      if($vehicleID!="selectall"){
           $Query="select * From caseData where assignedAmbulance='$vehicleID' and assignedTime between '$fromDate' and '$toDate'";
      }else if($vehicleID=="selectall" && $groupID!="selectall"){
            $devicesforgroup=getGroupDevices($accountID,$groupID);
            $Query="select * From caseData where assignedAmbulance  in ($devicesforgroup) and assignedTime between '$fromDate' and '$toDate'";
      }
      ...
      $m=1;
       ...
       {
          if($m==17){
              $result=$result."<th>Case Closure Time</th>";
          }
      //    else if($m==19 || $m==20 || $m==21 || $m==22 || $m==23){}
         else{
      ...
      while($row = mysqli_fetch_assoc($rs))
      {
         ...
         {
             if($y==4 || $y==5 || $y==10 || $y==16 || $y==17){
                 $result=$result."<td>".date('d-m-y H:i:s',$value)."</td>";
              }
           else if($y==24){
              $result=$result."<td>".str_replace(',','.',$value)."</td>";
             }
          else{
           ...
           $result=$result."<td><a href='php/caseTripMap.php?accountID=".$accountID."&caseID=".$row['caseID']."' target='_blank'>Route</a></td></tr>";
      }   
  /*
      while($row = mysqli_fetch_assoc($rs)) {
