...
//error_log($reportFormate.'Rep For');
    ...
    if (isset($_GET['fromDate'])) {
	 ...
	 $setup=parse_ini_file("setup.ini",true);// loading setup file for from time and to time
         $starttime=0;//$setup[$accountID.".fromtime"];
         $endtime=23;//$setup[$accountID.".totime"];
       //  $idleTime =10;// $setup[$accountID.".idletime"];
         $croneRequest="yes";
        if (isset($_GET['toDayDate'])) {
            $toDayDate=$_GET['toDayDate'];
         } else {
             $toDayDate=strtotime(date('d-M-Y',strtotime("-1 days")).'');
         }
    //    $toDayDate=1579415400;
        # $toDayDate=strtotime(date('d-M-Y',strtotime("-1 days")).'');
         $d=date('d',$toDayDate);
         $m=date('m',$toDayDate);
         $y=date('Y',$toDayDate);
 ...
 $days=array();
   $summarydata='';
      $summarydata1=$table1."<br><table border='1px' class='tablecss'><caption>Distance Summary Report</caption><thead  bgcolor='black' style='color:white'><th>Index</th> <th>District</th><th>Base Location</th><th>Asset</th><th>Crew Number</th><th>From Date</th><th>To Date</th>";
       if($croneRequest=="yes"){
                       $summarydata1=$table1."<br><table border='1px'><caption>Distance Summary Report</caption><thead><th>Index</th> <th>District</th><th>Base Location</th><th>Asset</th><th>Crew Number</th><th>Total Distance</th><th>Lat/Long</th><th>Operator</th><th>Status as on ".date('d-m-Y H:i:s',time())."</th></thead><tbody>";
   ...
   // $summaryIndx=0;
    ...
    for($devindx=0;$devindx<count($deviceArrya);$devindx++) {
        ...
        $indx=0;
        $gtsconnect = mysqli_connect($gtsserver, $gtsusername, $gtspassword) or die ("Unable to connect to the database1 : " . mysqli_error());
       ...
       $sql = "Select timestamp,accountID,deviceID,address,from_unixtime( timestamp, '%d-%m-%Y %H:%i:%s' ) as time1,statusCode,latitude,longitude,geozoneID,speedKPH from EventData where accountID='$accountID' and deviceID='".$deviceArrya[$devindx][4]."'  and timestamp BETWEEN $fromDate AND $toDate and latitude<>0 and longitude<>0";
error_log($sql);
          ...
          /* if($indx>=2){ //delete drift event compare 1 to second and first to third.
           ...
           if($indx>0 && $row['latitude']!= 0 && $row['longitude']!=0 && $data[$indx-1][5]!=0 && $data[$indx-1][6]!=0) {
                 ...
                 if(is_nan($data[$indx][7]) || ($data[$indx][7]/$difftimes)>0.025){//if distnace greater than 2 km with 5 minits then distnace=0 bzc drift point  
                  ...
                  if(date('d-m-Y',$data[$indx-1][10])!=date('d-m-Y',$row['timestamp'])){
                    $data[$indx][7]=0;
                  }
...
$data[$indx]= $data[$indx-1];
$data[$indx][9]=abs($deviceArrya[$devindx][6]-$toDate)/60;;
$data[$indx][3]=date('d-m-Y H:i:s',$toDate);
...
for($dindx=$fromDate;$dindx<$toDate;){
    ...
    if($reportType=="summary"){
         if($croneRequest!="yes"){
         $table=$summarydata1.$sumdays."<th>Total Distance</th><th>Lat/Long</th><th>Operator</th><th>Status as on ".date('d-m-Y H:i:s',time())."</th><th>Offline Time</th></thead><tbody>".$summarydata."<tr></tr><tr><td colspan='11'>*Off-road:Vehicle not Communicating from more than 12 hours *Not Responding:Never communicated </td></tr></tbody></table>
    </body>
    </html>";
        }else{
       ...
       $table=$summarydata1.$sumdays."<th>Total Distance</th><th>Lat/Long</th><th>Operator</th><th>Status as on ".date('d-m-Y H:i:s',time())."</th><th>Offline Time</th></thead><tbody>".$summarydata."<tr></tr><tr><td colspan='11'>*Off-road:Vehicle not Communicating from more than 12 hours *Not Responding:Never communicated </td></tr></tbody></table>".$table."<tr><td colspan='6'></td></tr></tbody></table>
    </body>
    </html>";
        }else{
...
function device_info($gtsserver, $gtsusername, $gtspassword,$gtsdb,$accounts,$userID,$vehicleID){
 ...
 //   echo $fromDate.'==='.$toDate.'===='.$now;
   ...
   while($row = mysqli_fetch_assoc($qry1)){
    ...
    //   if($userID == "admin" || $userID == "administrator" || $userID=="tracking" || $userID=="monitoring" || $userID == "avoidDuplecate" || $userID=="selectall"){
      ...
      /*  } else{
              if($row['groupID']==""){$row['groupID']="ungroup";}
               $result[$rowcount][8]="[".$row['deviceID']."][".$row['groupID']."][".$row['groupID']."]";
           }*/
