...
//http://up108.glovision.co/dashboard/php/casemanagementReporttest.php?vehicle=selectall&accountID=gvk-up-108&fromDate=1481826627&toDate=1481872097&userID=administrator&group=selectall&speedlimit=0&formate=formate
...
$table=$table."<table id='search' class='tablecss' border='1' style='font-family:Arial,Verdana,sans-serif;font-size:13;color:black;background:white;'><thead style='background:black;color:white'><th>District</th><th>BaseLocation</th><th>Ambulance No</th><th>Caller Name</th><th>Contact No</th><th>Case ID</th><th>Case Type</th><th>Case Time</th><th>Case Close Time</th><th>Case Location</th><th>Case Distance</th><th>Case Vehicle Status</th></thead><tbody>";
   ...
   // echo trim($dis11[2],'[').'   '.trim($dis11[1],'[');
     ...
     $VehicleStatus='';
     if($row['caseVehicleStatus']=="withevents"){
     $VehicleStatus="Online";
     } elseif($row['caseVehicleStatus']=="noevents") {
     $VehicleStatus="Offline";
     } else {
     $VehicleStatus="Needs Check";
     }
    $table=$table."<tr><td>".trim($dis11[2],'[')."</td><td>".trim($dis11[1],'[')."</td><td>".$row['deviceID']."</td><td>".$row['callerName']."</td><td>".$row['callerContact']."</td><td>".$row['caseID']."</td><td>".$row['caseType']."</td><td>".date('d-M-Y h:i:s a',$row['caseTimestamp'])."</td><td>".$closetime."</td><td>".$row['incidentLatitude']."/".$row['incidentLongitude']."</td><td>".($caseDist)." Kms</td><td class='orange'>".$VehicleStatus."</td></tr>";
