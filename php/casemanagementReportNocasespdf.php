...
$table=$table."<table id='search' class='tablecss' border='1' style='font-family:Arial,Verdana,sans-serif;font-size:13;color:black;background:white;'><tr style='background:black;color:white'><th>S.no</th><th>District</th><th>BaseLocation</th><th>Ambulance No</th></tr><tbody>";
         $groupID='';
         $description=''; 
...
$sno++;
 ...
 $newpdf=new PdfGeneretor();
       $newpdf->pdfcreation($table,$accountID.'_'.$group.'_'.$fromDate.'_nocases',null,"No Case Vehicles",date("d-m-Y",$fromDate));
$excel=new ExcelService();
$excel->generateExcel($table,$accountID.'_'.$group.'_'.$fromDate.'_nocases');
echo $table;

//        $newpdf=new PdfGeneretor();
  //     $newpdf->pdfcreation($table,$accountID.'_'.$group.'_'.time().'_nocases',null,"Case Manager No Cases","Monthly report");
