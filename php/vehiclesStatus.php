...
error_log("Request for dashboard stats".''.$vehicleId.'-'.$accountID.'-'.$fromDate.'-'.$userID.'-'.$vuserID.'-'.$toDate.'-'.$currentLocation.'-'.$offline.'-'.$selectedgroup);
    ...
    for($x=0;$x<count($lastLocaton);$x++) {
     ...
     //  error_log('after EventData');
      // while($row = mysqli_fetch_assoc($qry_result)) {
          if($lastLocaton[$x][10] !="0" && $lastLocaton[$x][11] !="0"){
               $ydata[$j][0]=0;//fuel votage
              if( $lastLocaton[$x][13]=="61715" || $lastLocaton[$x][13]=="61713" || $lastLocaton[$x][13]=="64842" || $lastLocaton[$x][13]=="64840" || $lastLocaton[$x][13]=="61477"
){
            $ydata[$j][1]="61717";
            $ydata[$j][2]="61717";


         }else if($lastLocaton[$x][13]=="61456" || $lastLocaton[$x][13]=="61716" || $lastLocaton[$x][13]=="61488" || $lastLocaton[$x][13]=="64784" || $lastLocaton[$x][13]=="61472"){
          ...
          if($ydata[$j][2]=="61714"){
              ...
              $ydata[$j][3]= floor($lastLocaton[$x][15]);
           ...
           $ydata[$j][4]=date('d-m-Y H:i:s',$lastLocaton[$x][1]);
           $ydata[$j][5]=intval($row['odometerKM']);
           $ydata[$j][6]=round($lastLocaton[$x][10],5);
           $ydata[$j][7]=round($lastLocaton[$x][11],5);
           $ydata[$j][8]=strtoupper($lastLocaton[$x][12]);
  ...
  $qry_result = mysqli_query($gtsconnect,$query) or die(mysqli_error($gtsconnect));
   ...
   while($row = mysqli_fetch_assoc($qry_result)){
    ...
    $lastLocaton[$i][12]=$row['maintNotes'];
      $lastLocaton[$i][13]=($row['statusCodeState']=="61714")?"61714":"61717";//$row['statusCodeState'];
...
function getGroups($gtsserver, $gtsusername, $gtspassword,$gtsdb,$accounts,$userID){
  ...
  // $vehicleDetails="select vehicleModel,vehicleMake,lastValidLatitude,lastValidLongitude,deviceID from Device where accountID='$accounts'";
    ...
    if($userID == "admin" || $userID == "administrator" || $userID == "tracking"  || $userID == "monitoring"){
        if($accounts=="gvk-ut-108" || $accountID=="gvk-ut-health"){
               $vehicleDetails="select groupID from DeviceGroup where accountID='$accounts'";
         }else{
...
$gtsconnect = mysqli_connect($gtsserver, $gtsusername, $gtspassword) or die ("Unable to connect to the database: " . mysqli_error($gtsconnect));
mysqli_select_db($gtsconnect,$gtsdb);
     $query="select deviceID from Device  where a.accountID='$accountID' and a.isActive=1";
...
function UpdateEventDataAddress($lat,$long,$accountID,$deviceID,$timestamp,$address,$idleLocStartTime,$idleLocEndTime) {
  ...
  function getGroupVehicle(){
     ...
     mysqli_select_db($gtsconnect,$gtsdb);
     $groupsforadmin="'ahmedabad','amreli','anand','aravalli','banaskantha','bharuch','bhavnagar','botad','chhotaudepur','dahod','devbhumidwarka','gandhinagar','girsomnath','jamnagar','junagadh','kheda','kutch','mahesana','mahisagar','morbi','narmada','navsari','panchmahals','patan','porbandar','rajkot','sabarkantha','surat','surendranagar','tapi','thedangs','vadodara','valsad'"; 
     $query="select a.groupID,a.deviceID from DeviceList a,Device b where a.accountID='$accountID' and a.groupID in ($groupsforadmin) and a.accountID=b.accountID and a.deviceID=b.deviceID and b.isActive=1";
     if($accountID=="gvk-ut-108" || $accountID=="gvk-ut-health"){
           $query="select a.groupID,a.deviceID from DeviceList a,Device b where a.accountID='$accountID' and a.accountID=b.accountID and a.deviceID=b.deviceID and b.isActive=1";
     }
