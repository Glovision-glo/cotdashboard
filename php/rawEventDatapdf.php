        ...
        font-size:13;
...
</style><center> <h3>Stoppage  Report</h3>AccountID:$accountID |Vehicle ID:$vehicleID <br> From:".date('d-M-Y h:i:s',$fromDate )."  To:".date('d-M-Y h:i:s',$toDate )." <table border='1' style='font-family:Arial,Verdana,sans-serif;font-size:13;color:black;background:white;' class='tablecss'><thead style='background:black;color:white'><th>S NO</th><th>Device ID</th><th>Status</th><th>Location</th><th>Start Time</th><th>End Time</th><th>Idle Time</th><th>Lat/Long</th><th>Distance (Km)</th></thead><tbody>";
         $startTime=0;
         $endTime=0;
         $idleStart='no';
         $count=0;
         ...
         for($evntindx=0;$evntindx<count($eventData);$evntindx++){
             ...
             if($idleStart=="yes" && $statusinfo=="Moving"){
                 ...
                 if(($dis/60)>=$idletime){
                     $table  =$table."<tr><td>".($count++)."</td><td>".strtoupper($eventData[$evntindx][1])."</td><td>Stopped</td><td>".$eventData[$evntindx][6]."</td><td>".date('d-m-Y H:i:s',$startTime)."</td><td>".$eventData[$evntindx][3]."</td><td>".TimeFormate($disdump)."</td><td>".round($eventData[$evntindx][8],5)."/".round($eventData[$evntindx][9],5)."</td><td>".round($totalDis,2)."</td></tr>";
                 }
             } 
...
//       mysqli_select_db($gtsdb, $gtsconnect);
       ...
       while($row = mysqli_fetch_assoc($rs)) {
          ...
          if($i==0){$startlatlong=round($row['latitude'],5).'-'.round($row['longitude'],5);
           ...
           //      echo $eventtoeventdistance.'difftime'.$difftimes.'   '.$eventData[$i][3].'<br>';
                 ...
                 if(is_nan($eventtoeventdistance) || ($eventtoeventdistance>5 && $difftimes<5)){//if distnace greater than 2 km with 5 minits then distnace=0 bzc drift point in 
                   ...
                   // echo  $eventData[$i][8].'  '.$eventData[$i][9].' '.$eventData[$i][3].' <br> ';                 
                   $i++;
               }
          }
         if($i==0){$i++;}
