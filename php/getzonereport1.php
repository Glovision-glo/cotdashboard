...
$toTime=$_GET['toTime'];
$fromDate=strtotime($fromDate." ".$fromTime);
$toDate=strtotime($toDate." ".$toTime);
...
if($reportFormate=="word"){
  ...
  header("Content-type: application/vnd.ms-excel");
   header("Content-Disposition: attachment;Filename=".$accountID."_".$group."_geozone_Report.xls");
...
.tablecss {
        ...
        font-size:13;
          ...
          <div id='printarea'><center><h3>Geozone Report </h3>User ID:<b style='font-size:16px' >$userID </b>|District:<b style='font-size:16px' >$group</b> <br> From:".date('d-M-Y h:i:s',$fromDate )."  To:".date('d-M-Y h:i:s',$toDate );
 $table="<center><br> <table border='1' class='tablecss'><thead style='background:black;color:white'><th>Vehicle ID</th><th>Group Name</th><th>Base Location</th><th>Geozone ID</th><th>Lat</th><th>Lon</th><th>Address</th><th>Arrival Time</th><th>Depart Time</th><th>Inside Elapsed</th></thead><tbody>";
$sno=1;
...
for($gpindx=0;$gpindx<count($groupArray);$gpindx++){
    ...
    for($lve=0;$lve<count($lastLocaton);$lve++){
        ...
        $tempArrivalData="";
        $laststatusCode=0;
       $tempArrivalTimeDiff=0;
        while($row = mysqli_fetch_assoc($qry_result))
        {   
          $laststatusCode=$row['statusCode'];
          //first depart
          if($arrivalStart=="" && $row['statusCode']=="62000"){
          
            $arvltime= getlatArrival($accountID,$lastLocaton[$lve][0],$fromDate); 
            $diff=0;
           
            if($arvltime!=0){
                 $diff=$row['timestamp']-$arvltime;
                 $arvltime=date('d-m-Y H:i:s',$arvltime);
  
            }else{
               $arvltime="";
             }
             $table .='<tr><td>'.strtoupper($row['deviceID']).'</td><td>'.strtoupper($vehicleGroup[strtolower($row['deviceID'])]).'</td><td> '.$lastLocaton[$lve][2].' </td><td>  '.strtoupper($row['geozoneID']).'</td><td> '.round($row['latitude'],5).'</td><td>  '.round($row['longitude'],5)."</td><td>".$row['address']."</td><td>".$arvltime."</td> <td>".date('d-m-Y H:i:s',$row['timestamp'])."</td><td>".secondsToTimeFormate($diff)."</td></tr>";
           $arrivalDepartCount++;
          }

        //arrival after arrival
         if($arrivalStart=="find" && $row['statusCode']=="61968"){
             $arrivalStart="";
                                 
              $table .='<tr><td>'.strtoupper($row['deviceID']).'</td><td>'.strtoupper($vehicleGroup[strtolower($row['deviceID'])]).'</td><td> '.$lastLocaton[$lve][2].' </td><td>  '.strtoupper($row['geozoneID']).'</td><td> '.round($row['latitude'],5).'</td><td>  '.round($row['longitude'],5)."</td><td>".$row['address']."</td><td>".date('d-m-Y H:i:s',$arrivalTime)."</td> <td>-------</td><td>--</td></tr>";
              $arrivalTime=0; 
         }
         
        //find arrival event
         if($arrivalStart=="" && $row['statusCode']=="61968"){

             $arrivalStart="find";
             $arrivalTime=$row['timestamp'];
              $diff=($toDate-$row['timestamp']);
              $tempArrivalTimeDiff=$diff;
              $tempArrivalData='<tr><td>'.strtoupper($row['deviceID']).'</td><td>'.strtoupper($vehicleGroup[strtolower($row['deviceID'])]).'</td><td> '.$lastLocaton[$lve][2].' </td><td>  '.strtoupper($row['geozoneID']).'</td><td> '.round($row['latitude'],5).'</td><td>  '.round($row['longitude'],5)."</td><td>".$row['address']."</td><td>".date('d-m-Y H:i:s',$row['timestamp'])."</td> <td>--------</td><td>".secondsToTimeFormate($diff)."</td></tr>";
          } 
          //find depart after arrival
         if($arrivalStart=="find" && $row['statusCode']=="62000"){
             $arrivalStart=""; 
              $totalTimeInzone=$totalTimeInzone+($row['timestamp']-$arrivalTime);
             $diff=($row['timestamp']-$arrivalTime); 
           $table .='<tr><td>'.strtoupper($row['deviceID']).'</td><td>'.strtoupper($vehicleGroup[strtolower($row['deviceID'])]).'</td><td> '.$lastLocaton[$lve][2].' </td><td>  '.strtoupper($row['geozoneID']).'</td><td> '.round($row['latitude'],5).'</td><td>  '.round($row['longitude'],5)."</td><td>".$row['address']."</td><td>".date('d-m-Y H:i:s',$arrivalTime)."</td> <td>".date('d-m-Y H:i:s',$row['timestamp'])."</td><td>".secondsToTimeFormate($diff)."</td></tr>";
             $arrivalTime=0;
             $arrivalDepartCount++; 
         }  
       ...
       if($laststatusCode=="61968"){
         $table .= $tempArrivalData;
         $totalTimeInzone=$totalTimeInzone+$tempArrivalTimeDiff;
         $arrivalDepartCount++;
       }
...
function getlatArrival($accountID,$deviceID,$fromdate){
    global $gtsserver, $gtsusername, $gtspassword,$gtsdb;
    $gtsconnect = mysqli_connect($gtsserver, $gtsusername, $gtspassword) or die ("Unable to connect to the database: " . mysqli_error($gtsconnect));
    mysqli_select_db($gtsconnect,$gtsdb);
     
    $query = "select max(timestamp) as arrivaltime from EventData where accountID='$accountID' and deviceID='".$deviceID."'  and timestamp < $fromdate  and statusCode in ('61968') order by timestamp asc";//
    $qry_result = mysqli_query($gtsconnect, $query) or die(mysqli_error($gtsconnect));
    $lastarrival=0;
     while($row = mysqli_fetch_assoc($qry_result)){
       $lastarrival=$row['arrivaltime'];
    }
    if($lastarrival==''){$lastarrival=0;}
