...
//$toTime=1471199400;
    ...
    </div>";//<a href='".$downloadurl."?accountID=".$accountID."&vehicleID=selectall&fromDate=".date('d-m-Y',$fromDate)."&toDate=".date('d-m-Y',$toDate)."&fromTime=".$fromTimee."&toTime=".$toTimee."&group=".$group."&reportFormate=excel&idletime=".$idleTime."&reportType=full'>Click to Download</a>
    
   
    $deviceArrya=array(array());
    $deviceArrya=device_info($accountID,$group,$deviceID);
    if($deviceArrya[0][0] != 'invalid' && $deviceID != "selectall"){
...
.tablecss {
        ...
        font-size:13;
             ...
             <center>Day Wise Vehicles Tracking  Report 
...
.tablecss {
        ...
        font-size:13;
             ...
             <center>Day Wise Mobile Tracking  Report 
             </div>Group :".strtoupper($group)."<br>";
        }
    }
...
.tablecss {
    ...
    if($deviceID != "selectall"){
        if(strpos($accountID,"gvk1")>-1 || strpos($accountID,"gog1")>-1){
            $table= $table."<center><table border='1px' class='tablecss'><caption>Detailed Distance Report</caption><thead bgcolor='black' style='color:white'><th>Index</th><th>Date</th><th>Start Time Of <br> The Day</th><th>End Time Of <br> The Day</th><th>Distance</th><th>Travel Route</th></thead><tbody>";
        }else{
        ...
        if(strpos($accountID,"gvk1")>-1 || strpos($accountID,"gog1")>-1) {
                       $table= $table."<center><table border='1px' class='tablecss' > <caption>Detailed Distance Report</caption><thead bgcolor='black' style='color:white'><th>Index</th><th>Date</th><th>Asset</th><th>Start Time Of <br> The Day</th><th>End Time Of <br> The Day</th><th>Distance</th><th>Travel Route</th></thead><tbody>";
        }else{       
          $table= $table."<center><table border='1px' class='tablecss' align='center'><caption>Detailed Distance Report</caption><thead bgcolor='black' style='color:white'><th>Index</th><th>Date</th><th>Asset</th><th>Idle Location For More<br> Than $idleTime mins</th><th>Total Idle Time</th><th>Start Time Of <br> The Day</th><th>End Time Of <br> The Day</th><th>Distance</th><th>Travel Route</th></thead><tbody>";
        }
    }
    $summaryIndx=0;
    for($devindx=0;$deviceArrya[0][0]!='invalid' && $devindx<count($deviceArrya);$devindx++) {
        ...
        $sql = "Select accountID,deviceID,address,from_unixtime( timestamp, '%d-%m-%Y %H:%i:%s' ) as time1,statusCode,latitude,longitude,geozoneID,speedKPH from EventData where accountID='$accountID' and deviceID='".$deviceArrya[$devindx][4]."' and timestamp BETWEEN $fromDate AND $toDate";
        error_log($sql);
        ...
        while($row = mysqli_fetch_assoc($qry_result)) {
            // error_log($row['accountID'].'   '.$row['deviceID'].'   '.$row['statusCode'].'    '.$row['time1'].'<br>');
        ...
        //      echo (abs(strtotime($data[$indx][3])-strtotime($data[$indx-1][3]))/60).'  dis:  '.$diffdis.'  '.$row['latitude'].'  '.$row['longitude'].'  '.$data[$indx][3].'<br>';
                if( $diffdis>5 && $difftimes>=5 && ($accountID=="gvkgjtest" || $accountID=="goghealth" || 
                        $accountID=="gvk-ut-health" || $accountID=="gvk-assam")){
                   //     echo $data[$indx][1].'   '.(abs(strtotime($data[$indx][3])-strtotime($data[$indx-1][3]))/60).'  '.$diffdis.'  '.$row['latitude'].'  '.$row['longitude'].'  '.$data[$indx-1][5].' '.$data[$indx-1][6].' callllllllllllllllllll dis<br>';
                    $googledistance= GetDrivingDistance($data[$indx-1][5],$data[$indx][5],$data[$indx-1][6],$data[$indx][6]);
                    // echo $googleddistance.'ffff<br>';
                    ...
                    $difftimes=abs(strtotime($data[$indx][3])-strtotime($data[$indx-1][3]))/60;
                    //echo $data[$indx][7];
                    //   echo $diss.' ddd '.$m.' dddd '.$googledistance.'  '.$data[$indx][3].'   '.$data[$indx-1][3].'   '.(abs(strtotime($data[$indx][3])-strtotime($data[$indx-1][3]))/60).'<br>';
                }else{
                    $data[$indx][7]=distance1($row['latitude'],$row['longitude'], $data[$indx-1][5], $data[$indx-1][6]);
                    // echo $data[$indx][7];              
                    if($accountID != "gvkrajasthan"){
                         $difftimes=abs(strtotime($data[$indx][3])-strtotime($data[$indx-1][3]))/60;
                         if(is_nan($data[$indx][7]) || (intval($data[$indx][7])>5 && $difftimes<5)) {
                             //if distnace greater than 2 km with 5 minits then distnace=0 bzc drift point  
                             $eventIncreament="no"; 
                             // delete duplicate events
                             error_log($data[$indx][7].' '.$difftimes.'-----'); 
                             $data[$indx][7]=0;
                         }
                     }
                }
            }
            /* if(is_nan($data[$indx][7]) || (intval($data[$indx][7])>10 && $difftimes<5)){
                 //if distnace greater than 2 km with 5 minits then distnace=0 bzc diff point  
                      $data[$indx][7]=0;
            }*/
            // if ($data[$indx][7] > 0) {
               //  error_log($row['latitude'].'new '.$row['longitude'].'new long'.$data[$indx-1][5].'old lat'. $data[$indx-1][6].'old long');
               //   error_log($indx.' '. $data[$indx][3].' '.$data[$indx][7]);
            //  } else {
            //       $data[$indx][7]=0;
            //  }
       ...
       geozoneList();
       // $days=array();
       $daywiseDistance=array(array()); 
       $numdays=0;
       for($dindx=$fromDate;$dindx<$toDate;) {
           $daywiseDistance[$numdays][0] = date('d/m',$dindx);
           $daywiseDistance[$numdays][1] = '-';
           $dindx=$dindx+86400;
           $numdays++;
       }
       for ($indx=0;$data[0][0]!='' && $indx<count($data);$indx++) { // idle for each day and print end of day.
           ...
           // error_log($indx.' '.$intvalue.' '.$min.' '.$sec);
           if ($indx+1 < count($data)) {
              //error_log($data[0][0].':'.$intvalue.':'.$data[$indx][3].'   '.$data[$indx+1][3].'   '.(abs(strtotime($data[$indx][3])-strtotime($data[$indx+1][3]))/60));
           }
           ...
           if($intvalue<$fromTimeIntval || ($lastEventOfTheDay == 1 && $intvalue>=$toTimeIntval)) {   
               // error_log('reject:'.$data[$indx][0].':'.$intvalue.':'.$data[$indx][3]);
               continue;
           }
           ...
           if ($startDate=='' || ($startDate!=date('d-m-Y',strtotime($data[$indx][3])) && ($firstEventOfTheDay == 0))){
               ...
               if($data[$indx][8]!='' && strtotime($data[$indx][8])==''){
...
//           error_log('lastevent check:'. $indx .' '. count($data). ' '. $lastEventOfTheDay .' ' . $startDate. ' '.date('d-m-Y',strtotime($data[$indx][3])) . ' ' . $intvalue. ' '. $toTimeIntval. ' '. $min.' '.$toTimemin.'  '.$sec. ' '.$toTimesec);
//           error_log('lastevent check2: '.date('d-m-Y',strtotime($data[$indx+1][3])));
           // end of the Day processing for the same date only for one last event.
           if ($indx > 0 && ($indx < count($data)) && $lastEventOfTheDay == 0 &&
                ((($startDate==date('d-m-Y',strtotime($data[$indx][3]))) && ($indx==count($data)-1)) || 
                 (($startDate==date('d-m-Y',strtotime($data[$indx][3]))) && (($intvalue>=$toTimeIntval) && ($min>=$toTimemin) && ($sec>=$toTimesec)))|| 
                 (($indx < count($data)-1) && ($startDate!=date('d-m-Y',strtotime($data[$indx+1][3])))))
              ) {
                error_log('lastevent:'.$data[$indx][0].':'.$intvalue.':'.$data[$indx][3]);
                $lastEventOfTheDay = 1;
                ...
                } else if (($indx < count($data)-1) && ($startDate!=date('d-m-Y',strtotime($data[$indx+1][3])))) {
                ...
                $tripUrl=$abspath."?accountID=".$accountID."&vehicleID=".$deviceArrya[$devindx][4].'&todate='.strtotime($endTime).'&fromdate='.strtotime($startDateTime)."&name=".$deviceArrya[$devindx][7]."&travelDistance=".round($perDayDistanceTravelled,2)."&destination=".str_replace(" ","-",$perdayendaddress)."&factory=".str_replace(" ","-",$perdaystartaddress)."&slatlong=".$startlatlong."&endlatlong=".$endlatlong."&idlelat=".$idlelat."&idlelong=".$idlelong."&onlyidles=no&request=itself";
                ...
                $daystring = date('d/m',strtotime($startDateTime));
                for ($dloop = 0;$dloop<$numdays; $dloop++) {
                    error_log('daystring vs daywise:'.$daystring.':'.$daywiseDistance[$dloop][0].':');
                    if ($daywiseDistance[$dloop][0] == $daystring) {
                        if ($perDayDistanceTravelled >0) {
                            $daywiseDistance[$dloop][1] = round($perDayDistanceTravelled,2);
                        } else {
                            $daywiseDistance[$dloop][1] = "-";
                        }
                        error_log('add days and distance:'.$dloop.':'.$daywiseDistance[$dloop][0].':'.$daywiseDistance[$dloop][1].':');
                        break;
                    }
                }

/*
                $daystring = date('d-m-Y',strtotime($startDateTime));
                $dayfound = 0;
                $dloop = 0;
                for ($dloop = 0;$dloop<count($daywiseDistance); $dloop++) {
                    if ($daywiseDistance[$dloop][0] == $daystring) {
                        $dayfound = 1;
                        break;
                    }
                ...
                if ($dayfound == 0) {
                     $daywiseDistance[$dloop][0] = $daystring;
                }
                $daywiseDistance[$dloop][1] = round($perDayDistanceTravelled,2);
                error_log('add days and distance:'.$dloop.':'.$daywiseDistance[$dloop][0].':'.$daywiseDistance[$dloop][1].':');
*/
...
//              $daywiseDistance[date('d-m-Y',strtotime($startDateTime))]=round($perDayDistanceTravelled,2);
                if(strpos($accountID,"gvk1")>-1 || strpos($accountID,"gog1")>-1){
                    $table=$table."<td>".$startAddress."<br>".$startDateTime."</td><td>".$endAddress."<br>".$endTime."</td><td>".round($perDayDistanceTravelled,2)." Kms</td><td><a href=".$tripUrl." target='_blank'>Route</a></td></tr>";
                } else {
                    // $days[count($days)]=date('d',strtotime($startDateTime));
                    $table=$table.$travelPath."</td><td>".secondsToTimeFormate($totalIdleTime*60)."</td><td>".$startAddress."<br>".$startDateTime."</td><td>".$endAddress."<br>".$endTime."</td><td>".round($perDayDistanceTravelled,2)." Kms</td><td><a href=".$tripUrl." target='_blank'>Route</a></td></tr>";
                }
                $idlelat='';
                $idlelong='';
                $perdaystartaddress='';
        ...
        error_log('distanceidle end of day loop '.$perDayDistanceTravelled.' '.$indx);
        if ($data[0][0] != '') {
           $slatlong=round($data[0][5],5).'-'.round($data[0][6],5);
           $elatlong=round($data[count($data)-1][5],5).'-'.round($data[count($data)-1][6],5);
           $tripUrl=$abspath."?accountID=".$accountID."&vehicleID=".$deviceArrya[$devindx][4].'&todate='.$toDate.'&fromdate='.$fromDate."&name=".$deviceArrya[$devindx][7]."&travelDistance=".round($totalDistanceTravelled,2)."&destination=".str_replace(" ","-",$data[count($data)-1][2])."&factory=".str_replace(" ","-",$data[0][2])."&slatlong=".$slatlong."&endlatlong=".$elatlong."&idlelat=".$allidlelatpervehilce."&idlelong=".$allidlelongpervehilce."&onlyidles=yes&request=itself";
        } else {
           $slatlong = 0;
           $elatlong = 0;
           $tripUrl = "";
        ...
        $sumdis='';
        $numdaysdist = 0;
        foreach($daywiseDistance as $key => $value) {
           if (!is_array($value)) {       
              error_log('1)'.$key .' '. $value);
           } else {   
              error_log('2)'.$key);
              foreach ($value as $key2 => $value2) {          
                 error_log('3)'.$key2 .' '. $value2);
                 if ($key2 == 1) {
                     $sumdis=$sumdis."<td>".$value2."</td>";
                     $numdaysdist++;
                 }
              }          
           }
        }
        for(;$numdaysdist<$numdays;$numdaysdist++) {
            $sumdis=$sumdis."<td>-</td>";
            $numdaysdist++;
        }

//        error_log('end of days and distance:'.$dindx.':');
        if($index>0 && $deviceID != "selectall"){
            ...
            if(strpos($accountID,"gvk")>-1 || strpos($accountID,"gog")>-1){
              ...
              if($croneRequest!="yes"){
                  $table=$table."<tr><th colspan='6'>Total Travelled Distance</th><th>".round($totalDistanceTravelled,2)." Kms</th><th><a href=".$tripUrl." target='_blank'>Route</a></th></tr>";
        ...
        if($index>0 && $deviceID == "selectall"){
             ...
             if(strpos($accountID,"gvk")>-1 || strpos($accountID,"gog")>-1){
                 ...
                 if($croneRequest!="yes"){
                    $table=$table."<tr><th colspan='7'>Total Travelled Distance</th><th>".round($totalDistanceTravelled,2)." Kms</th><th><a href=".$tripUrl." target='_blank'>Route</a></th></tr>";
                    $summarydata=$summarydata."<tr><td>".(++$summaryIndx)."</td><td>".$deviceArrya[$devindx][8]."</td><td>".trim($dis11[1],'[').' - '.$deviceArrya[$devindx][4]."</td><td>".$mobileNumber."</td><td>".$startDateforSummary."</td><td>".$endDateforSummary."</td><td>".round($totalDistanceTravelled,2)." Km</td></tr>";
                 }else{
                    $summarydata=$summarydata."<tr><td>".(++$summaryIndx)."</td><td>".$deviceArrya[$devindx][8]."</td><td>".trim($dis11[1],'[').' - '.$deviceArrya[$devindx][4]."</td><td>".$mobileNumber."</td><td>".round($totalDistanceTravelled,2)." Km</td></tr>";
                 }
...
//            $sumdays=$sumdays."<th>".$days[$dindx]."</th>";
    ...
    if($reportType=="summary"){
         if($croneRequest!="yes"){
            $table=$summarydata1.$sumdays."<th>Total Distance</th><th>Lat/Long</th><th>Operator</th><th>Status</th></thead><tbody>".$summarydata."<tr></tr><tr><td colspan='11'>*Off-road:Vehicle not Communicating from more than 12 hours *Not Responding:Never communicated </td></tr></tbody></table>
   ";
         }else{
         ...
         if($croneRequest!="yes"){
           $table=$summarydata1.$sumdays."<th>Total Distance</th><th>Lat/Long</th><th>Operator</th><th>Status</th></thead><tbody>".$summarydata."<tr></tr><tr><td colspan='11'>*Off-road:Vehicle not Communicating from more than 12 hours *Not Responding:Never communicated </td></tr></tbody></table><br>".$table."<tr><td colspan='6'></td></tr></tbody></table>";
   
//$table=$table."</tbody></table>".$summarydata1.$sumdays."<th>Total Distance</th><th>Lat/Long</th><th>Operator</th><th>Status</th></thead><tbody>".$summarydata."<tr></tr><tr><td colspan='11'>*Off-road:Vehicle not Communicating from more than 12 hours *Not Responding:Never communicated </td></tr></tbody></table>";
...
function device_info($accounts,$userID,$vehicleID)
{
   ...
   $vehicleDetails="select DISTINCT a.vehicleModel as vehicleModel,a.vehicleMake as vehicleMake,a.lastValidLatitude as lastValidLatitude,a.lastValidLongitude as lastValidLongitude,a.deviceID as deviceID,a.imeiNumber as imeinumber,lastEventTimestamp,a.simPhoneNumber as simPhoneNumber,b.groupID as groupID,a.simPhoneNumber as contactPhone,a.description as description ";
  $append = "from Device a,DeviceList b where a.accountID='$accounts' and b.groupID='$userID' and b.accountID='$accounts' and a.deviceID=b.deviceID ";
   if($userID=="admin" || $userID=="administrator" || $userID=="selectall"){
      $append="from Device a,DeviceList b where a.accountID='$accounts' and b.accountID='$accounts' and a.deviceID=b.deviceID ";
   }
   if($vehicleID!="selectall"){
      $append="from Device a,DeviceList b where a.accountID='$accounts' and b.groupID='$userID' and b.accountID='$accounts' and b.deviceID='$vehicleID' and a.deviceID='$vehicleID'";
   } 
   if(strpos($accounts,"gvk")>-1 || strpos($accounts,"gog")>-1){
      $append="from Device a,DeviceList b,Driver c where a.accountID='$accounts' and b.groupID='$userID' and b.accountID='$accounts' and c.accountID='$accounts' and a.deviceID=b.deviceID and a.deviceID=c.deviceID and c.accountID=a.accountID";
      if($userID=="admin" || $userID=="administrator" || $userID=="selectall"){
         $append="from Device a,DeviceList b,Driver c where a.accountID='$accounts' and b.accountID='$accounts' and a.deviceID=b.deviceID and a.deviceID=c.deviceID and c.accountID='$accounts'";
      }
      if($vehicleID!="selectall"){
         $append="from Device a,DeviceList b,Driver c where a.accountID='$accounts' and b.groupID='$userID' and b.accountID='$accounts' and b.deviceID='$vehicleID' and a.deviceID='$vehicleID' and c.deviceID='$vehicleID' and a.accountID='$accounts' and b.accountID='$accounts' and c.accountID='$accounts'";
      } 
   }
   $vehicleDetails .= $append. " group by a.deviceID ";
/*
...
function zoneID($lat1,$lon1){
  ...
  $geozoneID="";
  //error_log('zoneID:'.count($geozoneList).':'.$geozoneList[0][0].':');
  if ($geozoneList[0][0] == '') {
     return $geozoneID;
  }
